-- ============================================================
-- WePlaNet 로컬 DB 스키마 동기화 (증분, 데이터 보존)
-- ------------------------------------------------------------
-- 대상: main을 pull 받은 뒤 앱 기동 시
--       "Schema validation: missing column [...] in table [...]" /
--       "missing table [...]" 에러가 나는 팀원
-- 실행: MySQL Workbench에서 이 파일 전체 선택 후 실행 (Ctrl+Shift+Enter)
--
-- 기준: docs/weplanet_schema_full_reset.sql (이 파일은 그 스키마로부터 생성됨)
-- 동작:
--   1) 기존 테이블에 없는 컬럼만 ADD COLUMN
--   2) 없는 테이블만 CREATE TABLE IF NOT EXISTS
--   3) 배지 카탈로그(fan_badge) 중 없는 코드만 INSERT IGNORE
--   4) 마지막 [검증] 결과가 0행이면 동기화 완료
--
...
-- group_follow
CALL `wp_sync_add_column`('group_follow', 'fan_id', 'bigint NOT NULL COMMENT ''팬(users.id)''', 0);
CALL `wp_sync_add_column`('group_follow', 'group_id', 'bigint NOT NULL COMMENT ''팔로우한 그룹(artist_groups.id)''', 0);
CALL `wp_sync_add_column`('group_follow', 'created_at', 'datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) COMMENT ''팔로우 시각''', 1);
...
CALL `wp_sync_add_column`('fan_project_contribution', 'depositor_name', 'varbinary(255) NOT NULL COMMENT ''입금자명(현재 변환 저장, 추후 암호화)''', 0);
...
CALL `wp_sync_add_column`('fan_project_contribution', 'payment_status', 'varchar(30) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT ''READY'' COMMENT ''결제 상태: READY/PAID/FAILED 등''', 0);
...
-- ============================================================
-- [검증] 0행이면 동기화 완료. 행이 나오면 해당 테이블/컬럼을 확인하세요.
...
INSERT INTO `wp_expected_columns` VALUES
  ...
  ('group_follow', 'fan_id', 'bigint'),
  ('group_follow', 'group_id', 'bigint'),
  ('group_follow', 'created_at', 'datetime'),
  ...
  ('fan_project_contribution', 'depositor_name', 'varbinary'),
