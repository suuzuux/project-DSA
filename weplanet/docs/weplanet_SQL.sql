...
-- WEPLANET 통합 스키마
-- ------------------------------------------------------------
-- 실행법 (DB 없어도 됨 - CREATE DATABASE 포함)
--   mysql -uroot -proot < docs/weplanet_SQL.sql
--
-- !! 경고 : DROP TABLE 31개가 들어있다. 실행하면 기존 데이터가 전부 사라진다.
--
-- 변경 이력
--   2026-08-25 정휘원
--     fan_badge_ownership.group_id -> artist_id (FK: artist_groups -> users)
--     fan_project.group_id         -> artist_id (FK: artist_groups -> users)
--     이유: 팀 공통 커뮤니티(/community/{artistId})가 users.id(role=ARTIST)를 기준으로
--           동작하고, post.artist_id / membership.artist_id / chat_message.artist_id 도
--           모두 users(id)를 참조한다. 팬 프로젝트/뱃지만 artist_groups(id)를 보고 있어서
--           엔티티(Project.artist, FanBadgeOwnership.artist)와 어긋나 앱이 기동되지 않았다.
--     주의: 다른 group_id 컬럼(group_members / board_media / group_schedule / group_follow)은
--           artist_groups 기준 그대로다. 건드리지 않았음.
-- ============================================================
