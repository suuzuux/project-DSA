...
public class PortalController {
	...
	@GetMapping("/goods")
	public String goods(@AuthenticationPrincipal AuthenticatedUser principal,
						Model model,
						RedirectAttributes redirectAttributes) {
		...
		try {
			...
			model.addAttribute("goodsList", List.of());
			model.addAttribute("error",
					"굿즈 DB 테이블을 확인해주세요. docs/20260915_shop_goods.sql 과 docs/20260916_shop_goods_category_options.sql 을 적용한 뒤 서버를 재시작하세요.");
		}
		return "portal/goods";
	}
	...
	public String createGoods(@RequestParam String name,
		...
		try {
			...
			goodsService.create(artist, name, description, price, stockQuantity, officialUrl, status, thumbnail, categoryOptions);
	...
	public String updateGoods(@PathVariable Long goodsId,
		...
		try {
			...
			GoodsCategoryOptionsPayload categoryOptions = GoodsCategoryOptionsPayload.parse(categoryOptionsJson);
			goodsService.update(artist, goodsId, name, description, price, stockQuantity, officialUrl, status, thumbnail, categoryOptions);
