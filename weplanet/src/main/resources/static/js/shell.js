...
<!-- [머지 충돌 해결] main엔 이메일/전화번호가 "홍길동/hong22@gmail.com" 식으로 하드코딩된 목업이었고
         해지 버튼도 없었음. HEAD는 실데이터를 JS가 fetch로 채우는 방식 + 해지 폼이 있어서 HEAD 유지 -->
...
`;
  ...
  // [머지 충돌 해결] main엔 이 블록 자체가 없었음(폼 action이 안 채워지고, 상세 모달도 목업 그대로였음) -> HEAD 유지
